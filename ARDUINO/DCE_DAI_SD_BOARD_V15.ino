/********************************************************************************
  DCE_DAI_SD_BOARD_V15
  By............: Pierre Durant
  Date..........: 24/12/2020
  Version.......: 1.5
  Modification : autostart of the DCE device
  -------------------------------------------------------------------------------
  Wiring Layout
  -------------
...
void setup()
{
  ...
  int instruction[4];
  instruction[0] = 205; // CALL
  instruction[1] = 0;   //
  instruction[2] = 240; // 61440
  instruction[3] = 201; // RET

  // Configure pins

  pinMode(DataA0, OUTPUT);
  pinMode(DataA1, OUTPUT);
  pinMode(DataA2, OUTPUT);
  pinMode(DataA3, OUTPUT);
  pinMode(DataA4, OUTPUT);
  pinMode(DataA5, OUTPUT);
  pinMode(DataA6, OUTPUT);
  pinMode(DataA7, OUTPUT);

  pinMode(DataB0, INPUT);
  pinMode(DataB1, INPUT);
  pinMode(DataB2, INPUT);
  pinMode(DataB3, INPUT);
  pinMode(DataB4, INPUT);
  pinMode(DataB5, INPUT);
  pinMode(DataB6, INPUT);
  pinMode(DataB7, INPUT);

  pinMode(DataC0, INPUT);
  pinMode(DataC1, INPUT);
  pinMode(DataC2, INPUT);
  pinMode(DataC3, INPUT);

  pinMode(DataC4, OUTPUT);
  pinMode(DataC5, OUTPUT);
  pinMode(DataC6, OUTPUT);
  pinMode(DataC7, OUTPUT);
  ...
  for (int i = 0; i <= 20; i++) {
    if (digitalRead(BLUE) == 0) {
      digitalWrite(BLUE, 1);
      digitalWrite(BBLUE, 0);
      delay(70);
    } else {
      digitalWrite(BLUE, 0);
      digitalWrite(BBLUE, 1);
      delay(70);
    }
  }

  while (digitalRead(DataB0) == 0 ) {
    if (digitalRead(BLUE) == 0) {
      digitalWrite(BLUE, 1);
      digitalWrite(BBLUE, 0);
      delay(70);
  ...
  for (int i = 0; i <= 3; i++)
  {
    digitalWrite(BLUE, 1);
    delay(100);
    while (digitalRead(DataC2) == 0 ) {
    }
    Send_CharA(instruction[i]);
    digitalWrite(DataC7, HIGH);
    digitalWrite(BLUE, 0);
    delay(100);
    while (digitalRead(DataC2) == 1 ) {
    }
    if (i == 3) {
      digitalWrite(DataC5, LOW);
    }
    digitalWrite(DataC7, LOW);
  }
...
void Send_CharA(int byt)
{
  if (bitRead(byt, 0) == 0) {
    digitalWrite(DataA0, LOW);
  }
  else {
    digitalWrite(DataA0, HIGH);
  }
  if (bitRead(byt, 1) == 0) {
    digitalWrite(DataA1, LOW);
  }
  else {
    digitalWrite(DataA1, HIGH);
  }
  if (bitRead(byt, 2) == 0) {
    digitalWrite(DataA2, LOW);
  }
  else {
    digitalWrite(DataA2, HIGH);
  }
  if (bitRead(byt, 3) == 0) {
    digitalWrite(DataA3, LOW);
  }
  else {
    digitalWrite(DataA3, HIGH);
  }
  if (bitRead(byt, 4) == 0) {
    digitalWrite(DataA4, LOW);
  }
  else {
    digitalWrite(DataA4, HIGH);
  }
  if (bitRead(byt, 5) == 0) {
    digitalWrite(DataA5, LOW);
  }
  else {
    digitalWrite(DataA5, HIGH);
  }
  if (bitRead(byt, 6) == 0) {
    digitalWrite(DataA6, LOW);
  }
  else {
    digitalWrite(DataA6, HIGH);
  }
  if (bitRead(byt, 7) == 0) {
    digitalWrite(DataA7, LOW);
  }
  else {
    digitalWrite(DataA7, HIGH);
  }
}
...
void AboutSDDOS() {
  ...
  Send_Message_Line(" ");
  Send_Message_Line("SD-DOS for DAI 1.5");
  Send_Message_Line("By Pierre Durant - 2019 - 2020");
